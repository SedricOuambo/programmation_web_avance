...
export default function ContenuModule1() {
 return (
  <>
   ...
   <Paragraphe contenu="Les frameworks web ont révolutionné la façon dont nous construisons des applications web. Pour mieux comprendre leur importance, nous commencerons par examiner l'évolution ainsi que les limites du développement web original." />
   <Titre2 title="Les limites du développement web client original" />
   <Paragraphe contenu="Au début, le développement web était principalement basé sur des langages de balisage comme HTML et des styles CSS pour la présentation. Bien que ces technologies aient permis la création de pages web statiques, la gestion de la complexité croissante des applications web interactives était difficile." />
   <Paragraphe contenu="Ensuite, est arrivée l'ère du JavaScript côté client, qui a permis aux développeurs d'ajouter de l'interactivité et de la dynamique aux pages web. Cependant, cela a rapidement conduit à des problèmes de maintenabilité et de gestion de la complexité lorsque les applications sont devenues plus sophistiquées." />
   <Paragraphe contenu="Toutes ces limites du développement web client original finiront par être comblées avec l'avènment des frameworks car ces derniers possèdent de nombreux avantages." />

   <Titre2 title="Les avantages des frameworks web" />
   <Paragraphe contenu="Un framework web peut être défini comme étant un ensemble d'outils, de bibliothèques et de conventions qui permmettent aux développeurs de créer des applications web plus rapidement et plus efficacement, grace à  des fonctionnalités prêtes à l'emploi. Ces outils possèdent de nombreux avantages parmi lesquels on peut citer:" />
   <Item
   ...
   <Titre2 title="Les frameworks web client les plus utilisés" />
   <Paragraphe contenu="Au moment où ce contenu est produit, trois principaux frameworks sont utiliés par les développeurs pour de développement web client. Sans toutefois établir un ordre de popularité, ces frameworks sont les suivants:" />
   <Item
    item="React.js"
   ...
   <Item
    ...
    url="https://vuejs.org/"
   />
   <Paragraphe contenu="Dans le cadre de ce cours, nous utiliserons Next.js qui est un framework open source basé sur React, et développé par Vercel. Nous y reviendrons dans les prochaines sections." />
   ...
   <Paragraphe contenu="Dans la suite de notre cours, nous utiliserons la plateforme Next.js, qui est un framework basé sur la librairie React. Notre objectif principal consitera à développer la partie cliente d'une application web moderne." />
   <Remarque
    type="Remarque"
   ...
   />
   <Paragraphe contenu="Afin de réussir notre développement, nous aurons besoin d'installer quelques outils logiciels si cela n'est pas déjà fait." />

   <Titre2 title="Node.js" />
   <Paragraphe contenu="Le premier et l'indispensable outil à installer sera Node.js. C'est un environnement d'exécution JavaScript open source, permettant d'exécuter du code JavaScript dans un Terminal." />
   <LienExterne
   ...
   />
   <Paragraphe contenu="Pour savoir si Node.js est déjà installée et aussi savoir sa version, vous pouvez utiliser la commande suivante dans un Terminal :" />
   <CodeBlock
   ...
   <Titre2 title="clsx" />
   <Paragraphe contenu="clsx(classe extensible) est une petite bibliothèque JavaScript/TypeScript que nous utiliserons pour conditionner et combiner des noms de classes CSS de manière simple et efficace. Cela consistera à affecter différentes classes à un élément HTML à partir de certaines conditions préétablies." />
   <LienExterne
   ...
   <Titre2 title="ESLint" />
   <Paragraphe contenu="ESLint est un outil d'analyse statique du code pour JavaScript et TypeScript. Il nous permettra de détecter et de corriger les erreurs de syntaxe, les mauvaises pratiques, et les incohérences de style dans notre code." />
   <LienExterne url="https://eslint.org/" description="Comprendre ESLint" />

   <Titre2 title="Webpack" />
   <Paragraphe contenu="Webpack est un outil de construction qui sera utilisé par Next pour réaliser entre autres les tâches suivantes :" />
   <Item
   ...
   <CodeBlock
    type="terminal"
    contenu="npx create-next-app@latest"
    langage="jsx"
   />
   <Paragraphe contenu="Il suffit donc de répondre aux questions posées pour que le projet soit créé. Dans notre cas, voici les réponses à forunir : " />
   <Item
   ...
   />
   <Item item="Voulez-vous utiliser ESLint ? " description=" Réponse = YES" />
   <Item
   ...
   />
   <Item
    item="Voulez-vous utiliser Turbopack pour `next dev`? "
    description=" Réponse = YES"
   />
   <Item
   ...
   />
   <Titre2 title="Tester le fonctionnement de l'application" />
   <Paragraphe contenu="Pour tester le fonctionnement de l'application, il suffit de saisir les commandes qui vont suivre dans le Terminal. La première commande nous permettra de nous positionner dans le dossier de notre projet bilio-app et la deuxième permettra d'exécuter l'application." />
   <CodeBlock type="terminal" contenu="cd biblio-app" langage="jsx" />
   <CodeBlock type="terminal" contenu="npm run dev" langage="jsx" />
   <Paragraphe contenu="Pour finir, il suffit de placer le pointeur de la souris sur le lien 'http://localhost:3000' qui apparait, et de cliquer avec le bouton gauche de la souris pour voir la page d'accueil de l'application s'ouvrir avec le navigateur par défaut." />
   ...
   <Paragraphe contenu="Maintenant, est venu le moment pour nous d'explorer brièvement les différents fichiers générés lors de la création de notre projet. Si la création du projet s'est déroulée avec succès, votre projet aura la structure suivante : " />
   <DescriptionImage title="Structure du projet" url="structure.png" />
   <Titre2 title="Le dossier '/app'" />
   <Paragraphe contenu="Ce dossier contiendra la structure de base de notre application. Comme indiqué sur l'image précedente, ce dossier contiendra entre autres :" />
   <Item
   ...
   <Paragraphe contenu="Ce dossier contiendra tous les fichiers statiques de notre application. Il s'agit des fichiers qui ne seront pas compilés par Babel. Parmi ces fichiers, on peut citer les fichiers images, .json, .txt, les icônes et bien d'autres." />
   <Titre2 title="Le fichier '/package.json'" />
   <Paragraphe contenu="il contiendra les configurations du projet avec Node.js. D'autres configurations seront ajoutées avec l'évolution du cours." />
   <Titre2 title="Le fichier '/next.config.mjs'" />
   <Paragraphe contenu="C'est ce fichier qui contient les configurations de Next. Nous y reviendrons." />
   {/* // #endregion Structure d\'un projet React avec Next.js */}

   <Divider />
   {/* // #region Configurations de base pour notre projet */}
   <Titre1 title="Configurations de base pour notre projet" index="5" />
   <Paragraphe contenu="Jusqu'à présent, notre application, lorsqu'elle est exécutée, affiche des liens vers des pages de Vercel. Nous allons remplacer certains codes par les nôtres afin d'afficher simplement ' soyez les bienvenus sur biblio-app'. Pour cela, vous devez remplacer les codes des 4 fichiers qui vont suivre, par les codes fournis." />
   <Remarque
    type="Remarque"
   ...
   <CodeBlock
    chemin="/app/layout.js"
    contenu={getCode(code, "layout")}
    langage="jsx"
   />
   <Titre2 title="fichier '/app/page.js'" />
   <CodeBlock
    chemin="/app/page.js"
    contenu={getCode(code, "page")}
    langage="jsx"
   />
   <Titre2 title="fichier '/app/page.module.css'" />
   <CodeBlock
    chemin="/app/page.module.css"
    contenu={getCode(code, "page_module")}
    langage="css"
   />
   <Titre2 title="fichier '/app/globals.css'" />
   <CodeBlock
    chemin="/app/globals.css"
    contenu={getCode(code, "globals_css")}
    langage="css"
   />
   <DownloadLink url="./css/globals.css" item="globals.css complet" />
   <Remarque
    type="Remarque"
   ...
   <DownloadLink url="./pdf/projet-1.pdf" item="Télécharger le projet" />

   <Divider />
   <Titre1 title="Travail à faire" index="8" />
   <DownloadLink url="./pdf/semaine-1.pdf" item="Télécharger le travail" />
